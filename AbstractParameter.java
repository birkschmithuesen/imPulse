...
class AbstractParameter {
	...
	public boolean getChangedSinceReset() { /// < find out if the setting content was changed since "resetChange" was
...
interface OscMessageSink {
	...
	void writeToStream(DataOutputStream outStream); // write a description of the setting into a stream. used to build
													// parameter dump files
}
...
class OscMessageDistributor {
	static ArrayList<OscMessage> queuedMessages = new ArrayList<OscMessage>(); // messages accumulated from other
																				// threads

	static HashSet<OscMessageSink> allInstances = new HashSet<OscMessageSink>(); /// < freshly constructed instances are
																					/// added to this list to enable
...
class RemoteControlledFloatParameter extends FloatParameter implements OscMessageSink { //
	...
	// checks the adress pattern and data type of the incoming message, and sets the
	// currentValue of the receiving instance
	...
	// the method writes the names of the current remote Controlled Values and the
	// currentValues itself to a file on the harddrive. This file is used to
	// configurate the remote programm and to save the current setting
	public void writeToStream(DataOutputStream outStream) {
		String outData = "float" + "\t" + oscAdress + "\t" + "space for descripiton" + "\t" + getValue() + "\t"
				+ minValue + "\t" + maxValue + "\n";
		try {
...
class RemoteControlledIntParameter extends IntParameter implements OscMessageSink { //
	...
	// configurate the remote programm and to save the current setting
	public void writeToStream(DataOutputStream outStream) {
		String outData = "int" + "\t" + oscAdress + "\t" + "space for descripiton" + "\t" + getValue() + "\t" + minValue
				+ "\t" + maxValue + "\n";
		try {
			outStream.writeBytes(outData);
		} catch (IOException e) {
...
class RemoteControlledColorParameter extends ColorParameter implements OscMessageSink { //
	...
	RemoteControlledColorParameter(String _oscAdress, float _startHue, float _startSaturation, float _startBrightness) {
		super(_startHue, _startSaturation, _startBrightness); // super calls the constructor from the class, that it
																// extends from
	...
	public void writeToStream(DataOutputStream outStream) {
		...
		String outData = "float" + "\t" + oscAdress + "/Hue" + "\t" + "space for descripiton" + "\t" + currentHue + "\t"
				+ 0 + "\t" + 1 + "\n";
		try {
			outStream.writeBytes(outData);
		} catch (IOException e) {
		...
		outData = "float" + "\t" + oscAdress + "/Sat" + "\t" + "space for descripiton" + "\t" + currentSaturation + "\t"
				+ 0 + "\t" + 1 + "\n";
		try {
			outStream.writeBytes(outData);
		} catch (IOException e) {
		...
		outData = "float" + "\t" + oscAdress + "/Bright" + "\t" + "space for descripiton" + "\t" + currentBrightness
				+ "\t" + 0 + "\t" + 1 + "\n";
		try {
			outStream.writeBytes(outData);
		} catch (IOException e) {
