...
- `HitNodeOscThrottleTest` — der Notausgang gegen scsynth-FIFO-Überlauf (Vorfall 2026-08-02, `~163000` "command FIFO full"-Meldungen im Log, kein Ton mehr ohne Absturz). `/net/hitNode` hat anders als `/net/impulse` keinen Sender-Throttle: jeder Node-Treffer ging bislang ungedrosselt sofort raus. `HitNodeOscThrottle` legt Treffer stattdessen in eine FIFO-Warteschlange und liefert sie mit `/net/hitNode/rateHz` (Default 100 Hz, Bereich 1..200) aus — Bursts werden zeitlich geglättet, kein Treffer geht verloren, solange die Warteschlange nicht ihre Obergrenze erreicht (dann fällt der ÄLTESTE wartende Eintrag raus, nicht der neue — dieselbe Begründung wie bei `SplitStagger.MAX_PENDING`: das älteste Ereignis ist am weitesten von "jetzt sichtbar" entfernt). Ergänzt `~maxPolyphony` in `klangnetz_bells.scd` als zweite, unabhängige Sicherheitsebene — jene begrenzt gleichzeitig AKTIVE Stimmen, dieser Throttle die Rate EINGEHENDER OSC-Nachrichten.
