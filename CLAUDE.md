...
Die Parameter, die ein Preset umfasst: `masterVolume`, **`bellVolume`**,
**`droneVolume`**, `reverbMix`, `reverbRoom`, `reverbDamp`, `brightness`,
`detune`, `droneLpfMult`, `panSharpness`, **`treeBiasAmount`** (bis
2026-08-01 `regionBiasAmount`), **`melodyMode`**, **`melodyRootMidiNote`**,
**`melodyNumOctaves`**, `travelMix`,
`travelRq`, **`travelGrainRatio`**, `travelAmpScale`, `travelFreqBase`,
`travelSpeedRef`, `travelOctavesPerStep`, `travelSnap`, `travelFreqMin`,
