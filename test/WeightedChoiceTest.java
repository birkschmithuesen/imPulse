  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int i = 0; i <= 1000; i++) {
      Check.that("Gewicht 0 wird nie gezogen",
          WeightedChoice.pick(w, 5, 1, i/1000.0) != 0);
    }
    ...
    int n = 100000;
    for (int i = 0; i < n; i++) {
      counts[WeightedChoice.pick(w, 5, 1, (i + 0.5)/n)]++;
    }
    ...
    for (int i = 0; i < 40000; i++) {
      if (WeightedChoice.pick(w2, 2, 0, (i + 0.5)/40000.0) == 0) {
        erste++;
      }
    }
    ...
    Check.eq("alle Gewichte 0 gibt den Rueckfall",
        2, WeightedChoice.pick(new float[] { 0f, 0f, 0f, 0f }, 4, 2, 0.5));
    Check.eq("null-Array gibt den Rueckfall",
        2, WeightedChoice.pick(null, 4, 2, 0.5));
    Check.eq("count groesser als das Array gibt den Rueckfall",
        2, WeightedChoice.pick(new float[] { 1f, 1f }, 4, 2, 0.5));
    Check.eq("count 0 gibt den Rueckfall",
        2, WeightedChoice.pick(new float[] { 1f, 1f }, 0, 2, 0.5));
    Check.eq("negatives Gewicht wird nicht gezogen",
        1, WeightedChoice.pick(new float[] { -5f, 1f }, 2, 0, 0.5));
    Check.eq("NaN-Gewicht gilt als 0, nicht als Verfaelschung der Summe",
        1, WeightedChoice.pick(new float[] { Float.NaN, 1f }, 2, 0, 0.5));
    Check.eq("NaN-Zufall gibt den Rueckfall",
        2, WeightedChoice.pick(w, 5, 2, Double.NaN));
    ...
    for (int i = 0; i < 10000; i++) {
      if (WeightedChoice.pick(laenger, 2, 0, (i + 0.5)/10000.0) == 1) {
        zweiter++;
      }
    }
    ...
    Check.eq("Zufall unter 0 wird geklemmt", 1, WeightedChoice.pick(w, 5, 1, -1.0));
    Check.eq("Zufall ueber 1 wird geklemmt", 4, WeightedChoice.pick(w, 5, 1, 2.0));

    // Genau ein Gewicht: es gewinnt immer
    float[] nurEins = { 0f, 0f, 1f, 0f };
    for (int i = 0; i <= 100; i++) {
      Check.eq("einziges gewichtetes Element gewinnt immer",
          2, WeightedChoice.pick(nurEins, 4, 0, i/100.0));
    }
