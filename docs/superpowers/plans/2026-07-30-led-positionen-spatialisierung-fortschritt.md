...
**ERLEDIGT (2026-07-31, Birk vor Ort):** `~azimuthSign` und `~azimuthOffset`
sind gemessen — `azimuthSign = 1`, `azimuthOffset = -0.25`. Zusätzlich zwei
weitere Hardware-Probleme gefunden und behoben, die die Messung anfangs
verfälschten:

1. **scsynth bootete auf MME statt ASIO** (Windows-Default-Geräteauswahl
   griff daneben) — `s.options.device = "ASIO : ZOOM AMS-24 ASIO Driver"`
   jetzt explizit gesetzt.
2. **Hardware-Kabel-Reihenfolge des ZOOM AMS-24 ist keine einfache Rotation**
   der Decoder-Kanal-Reihenfolge (gemessen: hw0=rechts, hw1=links,
   hw2=hinten, hw3=vorn) — Kanal-Permutation in `\masterReverb` ergänzt.

Alle vier Positionen (vorn/rechts/hinten/links) wurden danach per Ohr am
Show-Rechner bestätigt korrekt zugeordnet. Verbleibende, dokumentierte
Einschränkung: Ambisonics 1. Ordnung streut prinzipbedingt auf
Nachbarkanäle (gemessen: dominanter Kanal trägt nur ~43% der Energie,
selbst bei voller Richtwirkung) — das ist keine offene Messung mehr,
sondern eine Verfahrensgrenze. Optionen für schärferes Panning:
`docs/ambisonics-sharper-panning-optionen.md`.
